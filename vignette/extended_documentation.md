...
This README provides a more detailed introduction to **LongAIRR**, including detailed
...
simple bash scripts. One advantage is the parallelization of processing steps that can be
run independently without affecting each other. Find more information about **Snakemake** [here](https://snakemake.github.io/).
The modular design of **LongAIRR** fits naturally into rule-based Snakemake workflows.
...
to GPU-processing, the here provided workflows expect FASTQ files as input, e.g. basecalled reads as output from **LongAIRR basecall**.

Each workflow includes one **snakefile**, containing rules for stepwise data-processing, and one **config.yaml**, where users provide all LongAIRR runtime parameters that are passed to the rules.
...
Essential **parameters** and **required additional files** must be provided by the user in the **config.yaml**; Exemplary for the spatial LongAIRR Snakemake Workflow for Visium V1 and Visium HD 3' datasets:
...
> [!Important]
> Spatial barcode whitelists for **Visium V1 datasets** can be obtained from [10x Genomics SpaceRanger](https://www.10xgenomics.com/support/software/space-ranger/latest) upon agreement to their terms and installing SpaceRanger locally. See [section above](#retrieving-10x-genomics-spatial-barcode-whitelists)

 ***Initial quality and length filtering***
 * [**Filter parameters**]: **`FILTER_MIN_QUAL`**, **`FILTER_MIN_L`**, **`FILTER_MAX_L`**, for initial quality and fixed length filtering. Set length filters to `-1` if not desired.
 ...
 * **`locus_igblast`**, valid values are `"ig"` and/or `"tr"` passed to igblast. Provide both, separated into new lines if you have a **combined BCR / TCR library** and need to split sequences by their corresponding locus.
 
 ***Performance / Speed***
 * **`COLLAPSE_N_SUBSAMPLE`** determines the maximal number of reads to keep for every sequence group. Sequence groups containing more reads will be subsampled with the specified random seed **`COLLAPSE_SEED`**.
...
```
snakemake --cores 12 -s path/to/longairr_example_workflow/snakemake/visium/snakefile_visium_hd --configfile path/to/longairr_example_workflow/snakemake/visium/config.yaml
```
...
The figure additionally shows the naming of the most important files, e.g., the main output from each longairr module, which would be used to pass the results from one module to the next one, when using LongAIRR manually and not with Snakemake.

___

### Demo Dataset

A small demo dataset resembling a Visium V1 dataset is saved in `/longairr_example_workflow/example_inputs/datasets/visium_v1_demo/basecall/demo_airr.fastq`.

To run the demo, first install LongAIRR and set up the reference databases (see [Installation section](../README.md)). After successful installation and activation of the corresponding conda environment,
follow the explanations provided above to adjust the provided **config.yaml** of the exemplary [**Spatial LongAIRR Snakemake Workflow**](#spatial-longairr-snakemake-workflow).
Especially all paths to the **input, output and anchor files** are **user-specific** and need to be adjusted by providing full paths to the corresponding files.

Example-paths can be found together with a Note within the **config.yaml** in `../longairr_example_workflow/snakemake/visium/config.yaml`

Also revisit the documentation on how to retrieve the spatial barcode whitelist for Visium V1 datasets from 10x Genomics SpaceRanger, [here](#retrieving-10x-genomics-spatial-barcode-whitelists).

> [!Important]
> 
> All provided sequences were artificially generated and do not represent real sequencing data, biological samples, or patient-derived material.
